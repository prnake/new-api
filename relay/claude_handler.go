...
func ClaudeHelper(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	...
	// Clean null fields in content blocks (e.g. "citations": null from Vertex Anthropic)
	request.CleanNullFieldsInMessages()
	...
	if baseModel, effortLevel, ok := reasoning.TrimEffortSuffix(request.Model); ok && effortLevel != "" &&
		model_setting.IsClaudeAdaptiveThinkingModel(request.Model) {
	...
	if model_setting.GetGlobalSettings().PassThroughRequestEnabled || info.ChannelSetting.PassThroughBodyEnabled {
		...
		if service.ShouldApplyAutoCache(c, info, request) {
			service.ApplyClaudeAutoCache(request)
		}
